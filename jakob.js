...
function scoreUpdate(card, isPlayer) {
  // Uppdaterar poäng för spelare och dealer
  if (card.value == 11) {
...
function dealersTurn(totalScoreDealer) {
  ...
  if (dealerPoints <= 16) {
    // den är fel..för den uppdaterar inte scoreboarden efter andra kortet
    console.log(dealerPoints); // dealerpoints är bara de förstta kortet inte totala..
    getRandomCard(false);
    if (dealerPoints > 2) {
      //när totala är uppdaterat så får ja ändra condittion här.
      console.log("hej");
      setTimeout(getRandomCard, 2000);
    }
  } else {
    stopGame();
  }
